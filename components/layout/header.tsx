...
export function Header() {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const pathname = usePathname();
  const { regionConfig, setShowSelector } = useRegion();
      ...
      <nav className="container-custom">
        <div className="flex items-center justify-between py-4">
          ...
          <div className="hidden lg:flex items-center space-x-3">
            <RegionSwitcher />
            <a href={`tel:${regionConfig.phone}`} className="flex items-center space-x-2 text-gray-300 hover:text-white transition-colors">
              <Phone className="w-4 h-4" />
              <span className="text-sm font-medium">{regionConfig.phoneDisplay}</span>
            </a>
            <a href={whatsappLink} target="_blank" rel="noopener noreferrer">
              <Button variant="primary" size="sm">
                Get Free Quote
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden py-4 border-t border-white/10">
            <div className="flex flex-col space-y-4">
              {/* Region button mobile */}
              <div className="px-4 pb-2">
                <button
                  onClick={() => { setShowSelector(true); setMobileMenuOpen(false); }}
                  className="w-full flex items-center justify-center space-x-2 py-2.5 rounded-xl text-sm font-medium"
                  style={{
                ...
                >
                  ...
                  <span>Serving {regionConfig.label}</span>
